        ...
        with open("usuarios.txt", "r") as file:#Se intenta abrir el archivo usuarios.txt en modo lectura r. Si el archivo no existe, se lanzara una excepcion para mas adelante
            for line in file:#Se itera sobre cada linea del archivo abierto.
                if line:  #verifica si la linea esta vacia
                    ...
                    user = parts[0].strip().lower()  #se pasa el name a minúsculas
                    if user == name:  
...
def openCloseSession(name, password, flag):
            ...
            if line:  
                try:
                    ...
                    if user == name and passw == password: #verifica que si el usuario y la contraseña digitada esta en el txt
                        ...
                        status = "conectado" if flag else "desconectado" #se actualiza el estado a true que es conenctado,si no lo es pasa lo contrario
    ...
    if user_found: #procede a actualzair el archivo
        with open("usuarios.txt", "w") as file: #sobrescribe lo que ya existia 
...
def updateScore(name, password, new_score):
    ...
    if updated:
        with open("usuarios.txt", "w") as file: #sobrescribe el dato"w"
            file.write("\n".join(lines) + "\n")
        return "Actualizado"
        ...
        for line in file:
            ...
            if line:  
                try:
                    ...
                    continue  
    return "error de contraseña o usuario"
            ...
            if questions:
                ...
                for line in questions:
                    ...
                    if line == "":  #Si es una linea vacia, significa que terminamos una pregunta
                        if question_buffer:
                            #agarra la primera pregunta random
                            question_text = question_buffer[0] 
                            options = question_buffer[1:5]  
                ...
                if not question_list:  #verifica si ya no hay preguntas en el archivo txt
                ...
                for question_data in question_list:
                    ...
                    #almacena la pregunta y respuesta del ususario
